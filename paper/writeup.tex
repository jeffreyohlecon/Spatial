...
\subsubsection{Idea 7: Distributional Effects of parking requirements}
...
your model.
\subsection{Mapping Monte to Redding Rossi Hansberg 2017}
\begin{enumerate}
...
\begin{enumerate}
\item $\alpha$, Cobb Douglas weight on consumption goods
\item $\epsilon$ dispersion of amenities
\item $\sigma$ CES Elasticity of Substitution
\item $F$ fixed cost of production (maybe?)
\end{enumerate}
Calibrated parameters
\begin{enumerate}
\item $\kappa_{ni}$, commuting costs
\item $d_{ni}$ goods trade cost
...
Solving for $A_i$
\begin{enumerate}
\item Can get a system of $N$ equations N unknown (wages, employment, residents, avg residential income and trade deficits are or computed fairly immediately from data)
\end{enumerate}
